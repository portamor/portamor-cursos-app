...
const postInstructor = async (req, res) => {
  try {
    ...
    if (!foundCourse) {
      throw new Error(
        `No se ha encontrado ningun curso con el ID: ${courseId}`
      );
    }
    ...
    res
      .status(201)
      .json({
        message: "Instructor creado con exito",
...
const getAllInstructor = async (req, res) => {
  try {
    ...
    if (!allInstructors.length)
      throw new Error("No se ha encontrado ningun instructor");

    res
      .status(200)
      .json({
        message: "Instructores encontrados con exito",
        data: allInstructors,
      });
  } catch (error) {
...
const putInstructor = async (req, res) => {
  try {
    ...
    const foundInstructor = await instructorService.getInstructorById(
      instructorId
    );
    ...
    res
      .status(200)
      .json({
        message: "Instructor actualizado con exito",
